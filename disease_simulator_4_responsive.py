...
# --- Refactored GUI Class (with responsiveness fix) ---

class SimulatorGUI:
    """
    Main GUI application for disease spread simulation with improved aesthetics and responsiveness.
    """
    def __init__(self, root):
        self.root = root
        self.root.title("🦠 Infectious Disease Spread Simulation (SIR Model)")
        # Start maximized or near-maximized for better visual
        ...
        # Initialize Tkinter variables (Necessary for the _add_setting structure)
    ...
    def _create_widgets(self):
        """Create an aesthetically pleasing and responsive GUI layout."""
        
        # --- Main Layout Configuration ---
        main_frame = ttk.Frame(self.root, padding="10 10 10 10")
        main_frame.pack(fill='both', expand=True)
        
        # Make the main frame rows/columns expandable
        main_frame.grid_columnconfigure(0, weight=0)  # Left (Controls) column has fixed width (weight 0)
        main_frame.grid_columnconfigure(1, weight=1)  # Right (Visualization) column expands (weight 1)
        main_frame.grid_rowconfigure(0, weight=1)     # Single row expands vertically

        # ------------------- Left Column: Controls and Stats (Fixed Width) -------------------
        ...
        # --- Statistics Display (Expands Vertically) ---
        stats_frame = ttk.LabelFrame(
            left_panel, text="📊 Current Status", padding="10 10 10 10")
        stats_frame.pack(fill='both', expand=True, pady=10) # FILL BOTH AND EXPAND TRUE FOR VERTICAL RESIZING
        ...
        # Log Text area takes up remaining vertical space
        ...
        stats_frame.grid_rowconfigure(row, weight=1) # THIS ROW EXPANDS VERTICALLY

        # ------------------- Right Column: Visualization and Plot (Expands Both Ways) -------------------
        right_panel = ttk.Frame(main_frame, padding="5")
        right_panel.grid(row=0, column=1, sticky='nsew', padx=10, pady=10)
        
        # Configure expansion for the right panel's contents
        right_panel.grid_rowconfigure(0, weight=3) # Canvas takes 3/4 of height
        right_panel.grid_rowconfigure(1, weight=1) # Plot takes 1/4 of height
        right_panel.grid_columnconfigure(0, weight=1) # Single column expands horizontally

        # --- Visualization Canvas (Expands) ---
        ...
        self.canvas.pack(fill='both', expand=True) # Essential for canvas inside LabelFrame
        ...
        self.plot_canvas_widget.pack(fill='both', expand=True) # Essential for plot inside LabelFrame
    ...
    def _on_canvas_resize(self, event):
        """Update canvas dimensions on resize and re-render network."""
        self.canvas_width = event.width
        self.canvas_height = event.height
        self.visualize_network()

    # --- Simulation Methods (Logic Unchanged) ---
    ...
    # --- Visualization and Statistics Methods (Logic Unchanged) ---
